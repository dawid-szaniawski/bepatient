...
from bepatient.waiter_src.executors.sql_executor import SQLExecutor


class TestSomeShit:
    def test_db(self, sqlite_db, select_all_from_user_query):
        data = sqlite_db.execute(select_all_from_user_query).fetchall()
        assert dictor(data, search="username") == ["WebLudus", "Dawid"]
        assert dictor(data, path="0.username") == "WebLudus"

    def test_db1(self, sqlite_db):
        query = "SELECT * from tests"
        data = sqlite_db.execute(query).fetchall()
        print(data)
